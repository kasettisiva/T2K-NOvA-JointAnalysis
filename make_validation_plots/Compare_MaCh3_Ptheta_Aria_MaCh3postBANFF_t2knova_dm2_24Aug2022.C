...
void SetRatioStyle(TH1* h, const char* title) {
    ...
    h->GetYaxis()->SetRangeUser(std::max(0.0, 0.7*h->GetBinContent(h->GetMinimumBin())), 2.5);
...
void GetRatioHist(TH1D* hp, TH1D* hm, TH1D* ha, TH1D* hmb, TLegend *leg, Binning binning, bool isDchi2, const char* output_name) {
    ...
    std::cout << " dm2_cred1sig.size() " << mdm2_cred1sig.size() << " " << " dm2_cred2sig.size() " << mdm2_cred2sig.size() << " pdm2_cred1sig.size() " << pdm2_cred1sig.size() << " pdm2_cred2sig.size() " << pdm2_cred2sig.size()<< std::endl;
    ...
    for(int i=0;i<pdm2_cred1sig.size();++i) {
        std::cout << " Ptheta 1 sig " << i << " " << pdm2_cred1sig[i][0] << " " << pdm2_cred1sig[i][1] << std::endl;
    }
    
    for(int i=0;i<pdm2_cred2sig.size();++i) {
        std::cout << " Ptheta 2 sig " << i << " " << pdm2_cred2sig[i][0] << " " << pdm2_cred2sig[i][1] << std::endl;
    }
...
void Compare_MaCh3_Ptheta_Aria_MaCh3postBANFF_t2knova_dm2_24Aug2022(const char* expt, const char* osc_par, const char* asimov, const char* outstring) {
    ...
    if(strstr(expt,"T2KNOvA") != NULL) {
        ...
        if(strstr(asimov,"Asimov1") != NULL) fm=new TFile("MaCh3_t2konly_Asimov1_03Sep2021_job3_ch0-999_red_reweighted.root");
        else if(strstr(asimov,"Asimov0") != NULL) fm=new TFile("~/Downloads/MaCh3_t2konly_Asimov0_03Sep2021_job3_ch0-999_red_reweighted.root"); //Asimov0
        else fm=new TFile("MaCh3_t2konly_Asimov1_03Sep2021_job3_ch0-999_red_reweighted.root");
    }
    ...
    if(strstr(expt,"T2KNOvA") != NULL) {
        ...
        if(strstr(asimov,"Asimov1") != NULL) fa=new TFile("~/Downloads/mcmcsamples_aria_t2konly_syst_burnin_asimov1_thinned1_postubrnin20000.root"); //Asimov1
        else if(strstr(asimov,"Asimov0") != NULL) fa=new TFile("~/Downloads/mcmcsamples_t2kcafana_joint_both_systs_noreactor_nddata_thinned1_postburnin20000.root"); //Asimov0
        else fa=new TFile("~/Downloads/mcmcsamples_aria_t2konly_syst_burnin_asimov1_thinned1_postubrnin20000.root");
    }
    ...
    if(strstr(expt,"T2KNOvA") != NULL) {
        ...
        if(strstr(asimov,"Asimov1") != NULL) fmbanff=new TFile("~/Downloads/t2konly_banff_job0-8_reweighted.root"); // RC 2019 Asimov1
        else if(strstr(asimov,"Asimov0") != NULL) fmbanff=new TFile("t2konly_postbanff_Asimov0_BURNIN_CUT.root"); //Asimov0
        else fmbanff=new TFile("~/Downloads/t2konly_banff_job0-8_reweighted.root"); // RC 2019 Asimov1
    }
    ...
    //++++++++++++++++++++++++++++++//
    //Get P-theta credible intervals
    
    TFile *fp;
    
    if(strstr(asimov,"Asimov1") != NULL) fp = new TFile(Form("credible_%s_%s_both.root",osc_par,expt));
    else if(strstr(asimov,"Asimov0") != NULL) fp = new TFile(Form("credible_%s_%s_asimov0_both.root",osc_par,expt));
    else fp = new TFile(Form("credible_%s_%s_both.root",osc_par,expt));
    
    TH1D* hp_no=(TH1D*)fp->Get("LL");
    TH1D* hp_io=(TH1D*)fp->Get("LL_inv");
    TH1D* hp_both_no=(TH1D*)fp->Get("LL_norm");
    TH1D* hp_both_io=(TH1D*)fp->Get("LL_inv_norm");
    
    if(strstr(osc_par,"dCP") != NULL || strstr(osc_par,"th23") != NULL) {
        hp_no->Rebin(51);
        hp_io->Rebin(51);
    }
    else {
    ...
    for(int i=0;i<binning.nbins;++i) {
        htemp_both_no->SetBinContent(i+1, hp_both_no->GetBinContent(i+1));
        htemp_no->SetBinContent(i+1, hp_no->GetBinContent(i+1));
    }
    
    TSpline3 *sp_io=new TSpline3(hp_io);
    TSpline3 *sp_both_io=new TSpline3(hp_both_io);
    ...
    for(int i=0;i<binning.nbins;++i) {
        ...
        //std::cout << " abs_dm31 " << abs_dm31 << " sp_io->Eval(abs_dm31) " << sp_io->Eval(abs_dm31) << " sp_both_io->Eval(abs_dm31) " << sp_both_io->Eval(abs_dm31) << std::endl;
        htemp_both_io->SetBinContent(i+1, std::max(0.0,sp_both_io->Eval(abs_dm31)));
        htemp_io->SetBinContent(i+1, std::max(0.0,sp_io->Eval(abs_dm31)));
    }
