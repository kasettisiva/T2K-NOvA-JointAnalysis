...
void GetRatioHist(TH1D* hp, TH1D* hm, TH1D* ha, TLegend *leg, Binning binning, bool isDchi2, const char* output_name) {
    ...
    if(!isDchi2) {
        for(unsigned int a=0;a<mdcp_cred1sig.size();a++) {
            drawCredInterval(pad1,mdcp_cred1sig[a][0],mdcp_cred1sig[a][1],0.6*mh_max,0.9*mh_max,hm->GetLineColor(),0.25,9);
        }
        
        for(unsigned int a=0;a<mdcp_cred2sig.size();a++) {
            drawCredInterval(pad1,mdcp_cred2sig[a][0],mdcp_cred2sig[a][1],0.6*mh_max,0.9*mh_max,hm->GetLineColor(),0.25,2);
        }
        
        for(unsigned int a=0;a<adcp_cred1sig.size();a++) {
            drawCredInterval(pad1,adcp_cred1sig[a][0],adcp_cred1sig[a][1],0.3*mh_max,0.6*mh_max,ha->GetLineColor(),0.25,9);
        }
        for(unsigned int a=0;a<adcp_cred2sig.size();a++) {
            drawCredInterval(pad1,adcp_cred2sig[a][0],adcp_cred2sig[a][1],0.3*mh_max,0.6*mh_max,ha->GetLineColor(),0.25,2);
        }
        
        for(unsigned int a=0;a<pdcp_cred1sig.size();a++) {
            drawCredInterval(pad1,pdcp_cred1sig[a][0],pdcp_cred1sig[a][1],0,0.3*mh_max,hp->GetLineColor(),0.25,9);
        }
        for(unsigned int a=0;a<pdcp_cred2sig.size();a++) {
            drawCredInterval(pad1,pdcp_cred2sig[a][0],pdcp_cred2sig[a][1],0,0.3*mh_max,hp->GetLineColor(),0.25,2);
        }
    }
...
void Compare_MaCh3_Ptheta_Aria_t2knova_18Aug2022(const char* expt, const char* osc_par, const char* outstring) {
 ...
 /*
    ...
    std::vector<std::array<double,2> > adcp_cred1sig = get1Dcred_disjoint(ha,0.6826);
    std::vector<std::array<double,2> > adcp_cred2sig = get1Dcred_disjoint(ha,0.9544);
    std::vector<std::array<double,2> > pdcp_cred1sig = get1Dcred_disjoint(htemp,0.6826);
    std::vector<std::array<double,2> > pdcp_cred2sig = get1Dcred_disjoint(htemp,0.9544);
    
    std::cout << " dcp_cred1sig.size() " << mdcp_cred1sig.size() << " " << " dcp_cred2sig.size() " << mdcp_cred2sig.size() << std::endl;
    for(int i=0;i<mdcp_cred1sig.size();++i) {
        std::cout << i << " 1 sig " << mdcp_cred1sig[i][0] << " " << mdcp_cred1sig[i][1] << std::endl;
    }
    
    TCanvas *ctemp=new TCanvas();
    Double_t mh_min = hm->GetBinContent(hm->GetMinimumBin());
    hm->Draw("hist");
    ha->Draw("hist same");
    htemp->Draw("hist same");
    
    for(unsigned int a=0;a<mdcp_cred1sig.size();a++) {
        drawCredInterval(ctemp,mdcp_cred1sig[a][0],mdcp_cred1sig[a][1],2.2*mh_min,2.9*mh_min,hm->GetLineColor(),0.25,9);
    }
    for(unsigned int a=0;a<mdcp_cred2sig.size();a++) {
        drawCredInterval(ctemp,mdcp_cred2sig[a][0],mdcp_cred2sig[a][1],2.2*mh_min,2.9*mh_min,hm->GetLineColor(),0.25,2);
    }
    
    for(unsigned int a=0;a<adcp_cred1sig.size();a++) {
        drawCredInterval(ctemp,adcp_cred1sig[a][0],adcp_cred1sig[a][1],1.5*mh_min,2.2*mh_min,ha->GetLineColor(),0.25,9);
    }
    for(unsigned int a=0;a<adcp_cred2sig.size();a++) {
        drawCredInterval(ctemp,adcp_cred2sig[a][0],adcp_cred2sig[a][1],1.5*mh_min,2.2*mh_min,ha->GetLineColor(),0.25,2);
    }
    
    for(unsigned int a=0;a<pdcp_cred1sig.size();a++) {
        drawCredInterval(ctemp,pdcp_cred1sig[a][0],pdcp_cred1sig[a][1],0.8*mh_min,1.5*mh_min,htemp->GetLineColor(),0.25,9);
    }
    for(unsigned int a=0;a<pdcp_cred2sig.size();a++) {
        drawCredInterval(ctemp,pdcp_cred2sig[a][0],pdcp_cred2sig[a][1],0.8*mh_min,1.5*mh_min,htemp->GetLineColor(),0.25,2);
    }
  */
