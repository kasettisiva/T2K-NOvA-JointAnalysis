...
void Compare_MaCh3_Ptheta_Aria_MaCh3postBANFF_t2knova_dm2_17July2022(const char* expt, const char* osc_par, const char* outstring) {
	...
	leg=new TLegend(0.25,0.65,0.4,0.85);
	leg->SetBorderSize(0);
	...
	hm_io->Draw("HIST");
    hmbanff_io->Draw("HIST same");
	ha_io->Draw("HIST same");
	htemp_io->Draw("HIST same");
	
	//c->SaveAs(Form("compare_credible_mach3_ptheta_aria_%s_both_%s_07July2022.pdf",osc_par,expt));
    c->SaveAs(Form("compare_credible_mach3_ptheta_aria_mach3postbanff_%s_both_%s_%s.pdf",osc_par,expt,outstring));
