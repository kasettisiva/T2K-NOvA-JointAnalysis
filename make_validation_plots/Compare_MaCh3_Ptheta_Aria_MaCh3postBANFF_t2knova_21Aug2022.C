...
void GetRatioHist(TH1D* hp, TH1D* hm, TH1D* ha, TH1D* hmb, TLegend *leg, Binning binning, bool isDchi2, const char* output_name) {
    ...
    std::vector<std::array<double,2> > mdcp_cred1sig = get1Dcred_disjoint(hm,0.6826);
    std::vector<std::array<double,2> > mdcp_cred2sig = get1Dcred_disjoint(hm,0.9544);
    std::vector<std::array<double,2> > mbdcp_cred1sig = get1Dcred_disjoint(hmb,0.6826);
    std::vector<std::array<double,2> > mbdcp_cred2sig = get1Dcred_disjoint(hmb,0.9544);
    std::vector<std::array<double,2> > adcp_cred1sig = get1Dcred_disjoint(ha,0.6826);
    std::vector<std::array<double,2> > adcp_cred2sig = get1Dcred_disjoint(ha,0.9544);
    std::vector<std::array<double,2> > pdcp_cred1sig = get1Dcred_disjoint(hp,0.6826);
    std::vector<std::array<double,2> > pdcp_cred2sig = get1Dcred_disjoint(hp,0.9544);
    
    std::cout << " dcp_cred1sig.size() " << mdcp_cred1sig.size() << " " << " dcp_cred2sig.size() " << mdcp_cred2sig.size() << std::endl;
    for(int i=0;i<mdcp_cred1sig.size();++i) {
        std::cout << " 1 sig " << i << " " << mdcp_cred1sig[i][0] << " " << mdcp_cred1sig[i][1] << std::endl;
    }
    
    for(int i=0;i<mdcp_cred2sig.size();++i) {
        std::cout << " 2 sig " << i << " " << mdcp_cred2sig[i][0] << " " << mdcp_cred2sig[i][1] << std::endl;
    }
    ...
    if(!isDchi2) {
        for(unsigned int a=0;a<mdcp_cred1sig.size();a++) {
            drawCredInterval(pad1,mdcp_cred1sig[a][0],mdcp_cred1sig[a][1],0.75*mh_max,1.0*mh_max,hm->GetLineColor(),0.25,9);
        }
        
        for(unsigned int a=0;a<mdcp_cred2sig.size();a++) {
            drawCredInterval(pad1,mdcp_cred2sig[a][0],mdcp_cred2sig[a][1],0.75*mh_max,1.0*mh_max,hm->GetLineColor(),0.25,2);
        }
        
        for(unsigned int a=0;a<mbdcp_cred1sig.size();a++) {
            drawCredInterval(pad1,mbdcp_cred1sig[a][0],mbdcp_cred1sig[a][1],0.25*mh_max,0.5*mh_max,hmb->GetLineColor(),0.25,9);
        }
        
        for(unsigned int a=0;a<mbdcp_cred2sig.size();a++) {
            drawCredInterval(pad1,mbdcp_cred2sig[a][0],mbdcp_cred2sig[a][1],0.25*mh_max,0.5*mh_max,hmb->GetLineColor(),0.25,2);
        }
        
        for(unsigned int a=0;a<adcp_cred1sig.size();a++) {
            drawCredInterval(pad1,adcp_cred1sig[a][0],adcp_cred1sig[a][1],0.5*mh_max,0.75*mh_max,ha->GetLineColor(),0.25,9);
        }
        for(unsigned int a=0;a<adcp_cred2sig.size();a++) {
            drawCredInterval(pad1,adcp_cred2sig[a][0],adcp_cred2sig[a][1],0.5*mh_max,0.75*mh_max,ha->GetLineColor(),0.25,2);
        }
        
        for(unsigned int a=0;a<pdcp_cred1sig.size();a++) {
            drawCredInterval(pad1,pdcp_cred1sig[a][0],pdcp_cred1sig[a][1],0,0.25*mh_max,hp->GetLineColor(),0.25,9);
        }
        for(unsigned int a=0;a<pdcp_cred2sig.size();a++) {
            drawCredInterval(pad1,pdcp_cred2sig[a][0],pdcp_cred2sig[a][1],0,0.25*mh_max,hp->GetLineColor(),0.25,2);
        }
    }
...
void Compare_MaCh3_Ptheta_Aria_MaCh3postBANFF_t2knova_21Aug2022(const char* expt, const char* osc_par, const char* outstring) {
...
//    TH1D* hmbanffdchi2_io = new TH1D("hmbanffdchi2_io","hmbanffdchi2_io", binning_io.nbins, binning_io.bin_min, binning_io.bin_max);
    ...
    TH1D* hmbanff=new TH1D("hmbanff",       Form("Both Hierarchies;%s;Posterior probability", binning.xtitle),
                      binning.nbins, binning.bin_min, binning.bin_max);
    TH1D* hmbanff_no =new TH1D("hmbanff_no",Form("Normal Hierarchy;%s;Posterior probability", binning.xtitle),
                          binning.nbins, binning.bin_min, binning.bin_max);
    ...
    hmbanffdchi2_no->SetLineColor(kBlack);
    hmbanffdchi2_io->SetLineColor(kBlack);
    ...
    GetRatioHist(htemp, hm, ha, hmbanff, (strstr(osc_par,"th23") != NULL)?leg:leg1, binning, false, Form("compare_credible_mach3_ptheta_aria_mach3postbanff_%s_both_%s_%s.pdf",osc_par,expt,outstring));
    ...
    GetRatioHist(htemp_no, hm_no, ha_no, hmbanff_no, (strstr(osc_par,"th23") != NULL)?leg:leg1, binning, false,  Form("compare_credible_mach3_ptheta_aria_mach3postbanff_%s_NO_%s_%s.pdf",osc_par,expt,outstring));
...
//    (strstr(osc_par,"th23") != NULL)?hmbanff_io->GetYaxis()->SetRangeUser(0,0.04):hmbanff_io->GetYaxis()->SetRangeUser(0,0.1);
    ...
    GetRatioHist(htemp_io, hm_io, ha_io, hmbanff_io, (strstr(osc_par,"th23") != NULL)?leg:leg1, binning, false,  Form("compare_credible_mach3_ptheta_aria_mach3postbanff_%s_IO_%s_%s.pdf",osc_par,expt,outstring));
    ...
    (strstr(osc_par,"th23") != NULL)?hpdchi2_no->GetYaxis()->SetRangeUser(0,30):hpdchi2_no->GetYaxis()->SetRangeUser(0,15);
	//hadchi2_no->GetXaxis()->SetMaxDigits(2);

    GetRatioHist(hpdchi2_no, hmdchi2_no, hadchi2_no, hmbanffdchi2_no, (strstr(osc_par,"th23") != NULL)?leg1:leg, binning, true, Form("compare_dchi2_mach3_ptheta_aria_mach3postbanff_%s_NO_%s_%s.pdf",osc_par,expt,outstring)); //((strstr(osc_par,"th23") != NULL)?true:false)
    ...
    GetRatioHist(hpdchi2_io, hmdchi2_io, hadchi2_io, hmbanffdchi2_io, (strstr(osc_par,"th23") != NULL)?leg1:leg, binning, true, Form("compare_dchi2_mach3_ptheta_aria_mach3postbanff_%s_IO_%s_%s.pdf",osc_par,expt,outstring));
