...
void GetRatioHist(TH1* hp, TH1* hm, TH1* ha, TLegend *leg, Binning binning, const char* output_name) {
...
void Compare_MaCh3_Ptheta_Aria_t2knova_10Aug2022(const char* expt, const char* osc_par, const char* outstring) {
    ...
    TH1D* hratio_ma = (TH1D*)hm->Clone();
    TH1D* hratio_pa = (TH1D*)htemp->Clone();
    ...
    TLegend* leg1;
    //leg=new TLegend(0.6,0.67,0.75,0.85);
    leg1=new TLegend(0.65,0.7,0.82,0.85);
    ...
    GetRatioHist(hm, htemp, ha, (strstr(osc_par,"th23") != NULL)?leg:leg1, binning, Form("compare_credible_mach3_ptheta_aria_%s_both_%s_%s.pdf",osc_par,expt,outstring));
    /*
    ...
    //hratio_ma->SetLineColor(kBlue);
    hratio_ma->SetMarkerStyle(20);
    hratio_ma->SetMarkerColor(kBlue);
    //hratio_pa->SetLineColor(kRed);
    hratio_pa->SetMarkerStyle(20);
    hratio_pa->SetMarkerColor(kRed);
...
*/
    ...
    TH1D* hratio_ma_no = (TH1D*)hm_no->Clone();
    TH1D* hratio_pa_no = (TH1D*)htemp_no->Clone();
    ...
    GetRatioHist(hm_no, htemp_no, ha_no, leg, binning, Form("compare_credible_mach3_ptheta_aria_%s_NO_%s_%s.pdf",osc_par,expt,outstring));
    
    /*
    TCanvas *c_no=new TCanvas("c_no","c_no",700,700);
    c_no->SetLeftMargin(0.14);
    
    TPad *pad1_no = new TPad("pad1_no", "pad1_no", 0, 0.3, 1, 1.0);
    SetTopPadStyle(pad1_no);
    ...
    c_no->cd();
    //TPad *pad2_no = new TPad("pad2_no", "pad2_no", 0.5,0,1,1);
    TPad *pad2_no = new TPad("pad2_no", "pad2_no", 0, 0.05, 1, 0.29);
    SetBottomPadStyle(pad2_no);
    
    //hratio_ma_no->SetLineColor(kBlue);
    hratio_ma_no->SetMarkerStyle(20);
    hratio_ma_no->SetMarkerColor(kBlue);
    //hratio_pa_no->SetLineColor(kRed);
    hratio_pa_no->SetMarkerStyle(20);
    hratio_pa_no->SetMarkerColor(kRed);
    
    hratio_ma_no->Divide(ha_no);
    hratio_pa_no->Divide(ha_no);
    
    //for(int i=0;i<hratio_ma_no->GetNbinsX();i++) std::cout << i << " " << hratio_ma_no->GetBinContent(i+1) << std::endl;
    
    SetRatioStyle(hratio_ma_no, binning.xtitle);
    
    hratio_ma_no->Draw("phist");
    hratio_pa_no->Draw("phistsame");
...
*/
    ...
    TH1D* hratio_ma_io = (TH1D*)hm_io->Clone();
    TH1D* hratio_pa_io = (TH1D*)htemp_io->Clone();
    ...
    GetRatioHist(hm_io, htemp_io, ha_io, leg, binning, Form("compare_credible_mach3_ptheta_aria_%s_IO_%s_%s.pdf",osc_par,expt,outstring));
    
    /*
    TCanvas *c_io=new TCanvas("c_io","c_io",700,700);
    c_io->SetLeftMargin(0.14);
    
    TPad *pad1_io = new TPad("pad1_io", "pad1_io", 0, 0.3, 1, 1.0);
    SetTopPadStyle(pad1_io);
    ...
    c_io->cd();
    //TPad *pad2 = new TPad("pad2", "pad2", 0.5,0,1,1);
    TPad *pad2_io = new TPad("pad2_io", "pad2_io", 0, 0.05, 1, 0.29);
    SetBottomPadStyle(pad2_io);
    
    //hratio_ma_io->SetLineColor(kBlue);
    hratio_ma_io->SetMarkerStyle(20);
    hratio_ma_io->SetMarkerColor(kBlue);
    //hratio_pa_io->SetLineColor(kRed);
    hratio_pa_io->SetMarkerStyle(20);
    hratio_pa_io->SetMarkerColor(kRed);
    
    hratio_ma_io->Divide(ha_io);
    hratio_pa_io->Divide(ha_io);
    
    SetRatioStyle(hratio_ma_io, binning.xtitle);
    
    hratio_ma_io->Draw("phist");
    hratio_pa_io->Draw("phistsame");
...
*/
    ...
    TH1D* hratio_ma_dno = (TH1D*)hmdchi2_no->Clone();
    TH1D* hratio_pa_dno = (TH1D*)hpdchi2_no->Clone();
    
    TH1D* hratio_ma_dio = (TH1D*)hmdchi2_io->Clone();
    TH1D* hratio_pa_dio = (TH1D*)hpdchi2_io->Clone();
    ...
    GetRatioHist(hpdchi2_no, hmdchi2_no, hadchi2_no, leg, binning, Form("compare_dchi2_mach3_ptheta_aria_%s_NO_%s_%s.pdf",osc_par,expt,outstring));
    
    /*
    TCanvas *c1=new TCanvas("c1","c1",700,700);
    
    TPad *pad1_dno = new TPad("pad1_dno", "pad1_dno", 0, 0.3, 1, 1.0);
    SetTopPadStyle(pad1_dno);
    ...
    c1->cd();
    //TPad *pad2 = new TPad("pad2", "pad2", 0.5,0,1,1);
    TPad *pad2_dno = new TPad("pad2_dno", "pad2_dno", 0, 0.05, 1, 0.29);
    SetBottomPadStyle(pad2_dno);
    
    //hratio_ma_dno->SetLineColor(kBlue);
    hratio_ma_dno->SetMarkerStyle(20);
    hratio_ma_dno->SetMarkerColor(kBlue);
    //hratio_pa_dno->SetLineColor(kRed);
    hratio_pa_dno->SetMarkerStyle(20);
    hratio_pa_dno->SetMarkerColor(kRed);
    
    hratio_ma_dno->Divide(hadchi2_no);
    hratio_pa_dno->Divide(hadchi2_no);
    
    SetRatioStyle(hratio_ma_dno, binning.xtitle);
    
    hratio_ma_dno->Draw("phist");
    hratio_pa_dno->Draw("phistsame");
	...
	*/
    ...
    (strstr(osc_par,"th23") != NULL)?hadchi2_io->GetYaxis()->SetRangeUser(0,25):hadchi2_io->GetYaxis()->SetRangeUser(0,7);
    ...
    GetRatioHist(hpdchi2_io, hmdchi2_io, hadchi2_io, leg, binning, Form("compare_dchi2_mach3_ptheta_aria_%s_IO_%s_%s.pdf",osc_par,expt,outstring));
	
    /*
    TCanvas *c2=new TCanvas("c2","c2",700,700);
    
    TPad *pad1_dio = new TPad("pad1_dio", "pad1_dio", 0, 0.3, 1, 1.0);
    SetTopPadStyle(pad1_dio);
    ...
    c2->cd();
    //TPad *pad2 = new TPad("pad2", "pad2", 0.5,0,1,1);
    TPad *pad2_dio = new TPad("pad2_dio", "pad2_dio", 0, 0.05, 1, 0.29);
    SetBottomPadStyle(pad2_dio);
    
    //hratio_ma_dio->SetLineColor(kBlue);
    hratio_ma_dio->SetMarkerStyle(20);
    hratio_ma_dio->SetMarkerColor(kBlue);
    //hratio_pa_dio->SetLineColor(kRed);
    hratio_pa_dio->SetMarkerStyle(20);
    hratio_pa_dio->SetMarkerColor(kRed);
    
    hratio_ma_dio->Divide(hadchi2_io);
    hratio_pa_dio->Divide(hadchi2_io);
    
    SetRatioStyle(hratio_ma_dio, binning.xtitle);
    
    hratio_ma_dio->Draw("phist");
    hratio_pa_dio->Draw("phistsame");
